...
let validate number =
  ...
  if check_digit <> String.make 1 number.[len - 1] then raise Invalid_checksum;
  number
...
let format number =
  ...
  (* Last digit is always the check digit *)
  let check_digit = String.make 1 number.[len - 1] in
  let main_part = String.sub number 0 (len - 1) in
